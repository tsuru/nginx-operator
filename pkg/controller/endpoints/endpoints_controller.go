...
func (r *reconcileEndpoints) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		if createEndpoints {
			r.recorder.Eventf(newEndpoints, corev1.EventTypeWarning, "FailedToCreateEndpoint", "Failed to create endpoint for service %v/%v: %v", service.Namespace, service.Name, err)
		} else {
			r.recorder.Eventf(newEndpoints, corev1.EventTypeWarning, "FailedToUpdateEndpoint", "Failed to update endpoint %v/%v: %v", service.Namespace, service.Name, err)
		}
